...
// trigger trade with 50%
const entryAmountPercent = 50;

// This varies per asset - get this from exchange info, do not hardcode it
...
function trimToDecimalPlaces(number: number, precision: number): number {
  const array: string[] = number.toString().split('.');
...
(async () => {
  try {
    ...
    console.log(`Testing buy order syntax (optional): `, buyOrderRequest);
    ...
    /**
     * Process buy fills and submit sell amount
     */
